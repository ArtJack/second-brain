...
class TestIdentityIsContent:
    def test_the_same_text_from_two_paths_is_one_point(self, tmp_path, store):
        from secondbrain.ingest import ingest_paths

        first = tmp_path / "a" / "note.md"
        second = tmp_path / "b" / "note-copy.md"
        ...
        assert store.count() == 1, "copying a file must not double the corpus"
